...
async function chat(userInput, vectorStore) {
  const relevantChunks = await findRelevantChunks(userInput, vectorStore);
  ...
  const response = await anthropic.messages.create({
    ...
    say "I don't have that information." Remember previous 
...
async function main() {
  ...
  console.log('Document loaded! Ask me anything. Type "exit" to quit.\n');
  ...
  const askQuestion = () => {
    rl.question("You: ", async (userInput) => {
      ...
      const reply = await chat(userInput, vectorStore);
      console.log(`\nAssistant: ${reply}\n`);
      askQuestion();
    });
  };
